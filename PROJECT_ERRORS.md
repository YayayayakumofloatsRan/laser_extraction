...
- If the validation error jumps back from the current `0.053 um` baseline, first verify the `Cam_pos -> Laser` pairing, the world-frame reference image, and whether `stripe_extraction_params` drifted away from `global_centroid + median+gaussian + threshold_ratio=0.25`.
